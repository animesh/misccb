...
# $Revision: 1.1.6.32 $ $Date: 2009/11/13 04:37:19 $
...
($FileRev)='$Revision: 1.1.6.32 $'=~/([\d.]+)/;
$cmdline=join ' ', @ARGV;
$exitcode=0;
parsArgs(debug=>0,outfile=>'-',calltype=>'cdecl',allsrc=>0);
$outfile=$options{outfile};
$GenThunkFile=exists $options{thunkfile};
...
%baseTypes = qw(int8_t int8 int8_T int8 char int8 short int16 int int32 long long
              __int64 int64 longlong int64 longlongint int64 shortint int16 longint long); 
...
#elif (__SUNPRO_CC >= 0x550) || (__SUNPRO_C >= 0x550)
  #define DLL_EXPORT_SYM __symbolic
#elif __GNUC__ >= 4
...
END_HEADER
 ############### End of header code
    ...
    my $headername=$inputfile;
    $headername=~s/\.i$/\.h/;
    print THUNKFILE "#include \"$headername\"\n";
#Now set up special type mappings for thunk file
    my @mltypes=qw(int8 int16 int32 int64 );
    my @ctypes=qw(char short int);
    push (@ctypes,'long long');
    my $i;
    for ($i=0;$i<@ctypes;$i++) {
...
$srcLine=0;  #if no line statements default to line 1
$packing=undef;
for (@ARGV) {
    s/([\w\s]+)\.[\w\s]*$/$1/;
...
while (<INFILE>)
{
    ...
    if (/^#/) {
     ...
     if (/^#line\s+(\d+)\s+\"(.*)\"/ || /^# (\d+) \"(.*)\"/ ) {
        ...
        if ($2 ne $srcFile) {
            $srcFile=$2;
            $inSrcFile=/\b$SrcFiles\b/i if !$options{allsrc};
...
#        next;
	...
	if (!$inSrcFile) {
        ...
        if ($GenThunkFile && ($calltype ne 'matlabcall')) {
...
sub ParseType {
    ...
    if ($type=~/^\w+$/ || $type=~/\*$/) { # simple case one word or trailing ptr
        ...
        #three or more words including keyword(s)?
        $varName= $3;
        my $tt=$1.$2;
        $tt =~ s/^(.*)\bunsigned \b(.*)$/unsigned $1$2/;
        $type=GetUddType($tt);
        print "from '$_[0]' got type of $type tt=$tt varName='$varName'\n" if exists $debug{'longtypes'};
...
sub cleanupType{
    ...
    $type =~ s/\b(?:_{0,2}const|signed|struct|enum)\b//g;
...
sub addFunctionThunk {
    ...
    for  ($_=0;$_ < @$paramsIn;$_++) { #fix up input types to 
        ...
        #what about structs by value need definition ? include header???
