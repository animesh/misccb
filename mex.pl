...
sub options_file
{
    ...
    if ($OPTFILE_NAME =~ /msvc90free/ && $ENV{'MWE_VS90FREE_COMNTOOLS'}) {
...
sub parse_common_dash_args
{
    ...
    ARGTYPE: { 
      ...
      /^-D\S*$/ && do {
        if ($_ eq "-DV5_COMPAT") {
            expire("The V5_COMPAT macro is no longer supported.");
...
end_fortran_cannot_change_entrypt
    ...
    /^inline_deprecated$/ && print(<<"end_inline_deprecated") && last DESCRIPTION;
**************************************************************************
  Warning: -inline is deprecated and will be removed in a future release.
**************************************************************************

end_inline_deprecated
...
sub parse_mex_args
{
    ...
    for (;$_=shift(@ARGV);) {
        ...
        ARGTYPE: {
          ...
          /^-V5$/ && do {
              expire("The -V5 option is no longer supported.");
              last ARGTYPE;
          };
