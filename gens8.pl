...
#!/usr/bin/perl
if( @ARGV ne 4){die "\nUSAGE\t\"ProgName MultSeqFile windowlength ?baseperiodicity ExonIntronCutoffValue\"\t\n\n\n";}
$file = shift @ARGV;
$ws = shift @ARGV;
$n= shift @ARGV;
$eicuto=shift @ARGV;
...
$c=$subseq=~s/C/C/g;$a=$subseq=~s/A/A/g;$g=$subseq=~s/G/G/g;$t=$subseq=~s/T/T/g;
	#print "$subseq\t$a\t$t\t$g\t$c\n";
		...
		if($ptnr >= 4.0)
                   	{
			#print "Window Analysis of $sname (range:$c2 to $ll):Peak 2 NoiseRatio is $ptnr\n";
			#print "now doing Fourier Spect of the Region...\n";
			#FT($c2,$ll);
			$midpoint=$c2+($ws/2)-1;push(@regi,$midpoint);
			#print "$midpoint\n";
   			#$region{$midpoint}=$ptnr;
			}
                else
                      	{
	...
	$relen=@regi;#print "$relen\n";
	for($c10=0;$c10<$#regi;$c10++){
	$startposprev=@regi[$c10]-$ws/2+1;
	$stopposnext=@regi[$c10+1]+$ws/2;
	$startposnext=@regi[$c10+1]-$ws/2+1;
	$stopposprev=@regi[$c10]+$ws/2;
	$dist=$startposnext-$stopposprev;
	$cnew=$c10+2;
        	if($dist le $eicuto){
			if($flag eq 0){
			#print "\nSTART:\t$startposprev\t";
                        push(@start,$startposprev);
                        $flag=1;
                        }
				elsif ($cnew == $relen) {
				print "END:\t$stopposnext\tELSEIF\n" ;
                                push(@stop,$stopposnext);
                                $flag=0;
                        	}
	...
	print "cont $cnew relen $relen\n"; # End else
	}
	for($c11=0;$c11<=$#start;$c11++){
	FT(@start[$c11],@stop[$c11]);
	#foreach $val (keys %region){
	#print "$val =>$region{$val}\n";
	}
	# End else
	sub FT {
	$st=shift;
	...
	#foreach $val (keys %region){delete $region{$val};}
	undef %region;undef @start;undef @stop;undef @regi;
}
