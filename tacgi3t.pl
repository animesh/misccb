...
/* $Id: tacgi3t.pl,v 1.2 2001/10/15 19:12:22 hjm Exp $  */
...
$htmlpathtoform = "tacg3/formt.html";  # Where/what the form is
...
$tacgapp = "/usr/local/bin/tacg3t";          # The app name of the tacg3 exe
...
#    print "<b> @GB_tokens : $GENBANK : $NGBE </b>";  
...
if (defined $GRAFMAP) {
   $tacgcmdline .= "--ps 0 --tmppath $dirname "; # only add  if it's other than the default
}

if (defined $LOGDEGENS) {
   $tacgcmdline .= "--logdegens "; # only add  if it's other than the default
}
