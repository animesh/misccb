...
for($d3=1;$d3<=$tots3;$d3++){
$m+=$plan[$d3][$d2];
}$name=$plan[0][$d2];$mean{$name}=($m/($d3-1));1/1;
$m=0;
}
for($d2=1;$d2<=$tots2;$d2++){$name=$plan[0][$d2];
for($d3=1;$d3<=$tots3;$d3++){
$m11+=(($plan[$d3][$d2]-$mean{$name})**2);
print "$plan[$d3][$d2]\t";}$sd{$name}=sqrt($m11/($d3-1));1/1;
print "$d3\t$m11\n";$m11=0;
				...
				for($d2=1;$d2<=$tots2;$d2++){$n2=$plan[0][$d2];print "seq1=>$n1\tseq2=>$n2\n";
					if($n1 eq $n2){$rmatix[$d22][$d2]=0;$rmatixn[$d22][$d2]=0;
							...
							$nnn+=(($plan[$d3][$d22]-$mean{$n1})*($plan[$d3][$d2]-$mean{$n2})/($sd{$n1}*$sd{$n2}));1/1;
...
#system "aninj.exe";system "write aninj.txt";system "aniupgma.exe";system "write aniupgma.txt";
