#!/usr/bin/perl
#$rcs = ' $Id: search.pl,v 1.31 2001/03/03 23:45:35 daniel Exp $ ' ;
...
sub main {
  ...
  #my $elapsed_time = sprintf("%.2fsec", Time::HiRes::tv_interval($start_time)) if( $start_time );
  #print $elapsed_time;
  print $html;
  log_query($query);
}
...
sub cast_template {
  ...
  $h{'query_str'}   = $q;		# it's escaped above already
...
sub term_emphasize {
  ...
  $str =~ s/\b(\Q$term\E|\Q$term_no_accents\E)\b/<strong>$1<\/strong>/igs;
  return $str;
}
