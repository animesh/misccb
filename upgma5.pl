...
$file=<STDIN>;
chomp $file;   $least=1;
print "-(1)For Hamming Distance Matrix calculation enter \t1\n";
...
$t2=$co1-1;
print "phylogenetic tree for sequence #no. starting from 0 to $t2\n";
system "./a.out";
for ($i=0;$i<$co1;$i++)
{
for ($j=0;$j<$co1;$j++)
...
$sum=$mism[$i][$j]-($sum1/($co1-2))-($sum2/($co1-2));
$nmism[$i][$j]=abs ($sum);
}
}
for($c=0;$c<$co1;$c++)
