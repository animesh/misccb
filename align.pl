...
# aligns 2 200-character sequences in 300 seconds
...
align - global optimal sequence alignment

=head1 SYNOPSIS

B<align> [B<-a>] [-p m,m,o,e] [<-s>] [B<-v>] F<file1> F<file2>

B<align> B<-t> [B<-v>]

=head1 DESCRIPTION

B<align> finds the global optimal alignment of the sequences in
F<file1> and F<file2>, subject to the payoff matrix.

=head1 ALGORITHM

B<align> uses the Needleman-Wunsch dynamic programming algorithm,
which runs in O(a*b*(a+b)), where a and b are the lengths of the 
...
=item B<-p> I<match>B<,>I<mismatch>B<,>I<open>B<,>I<extend>

Specifies the payoffs for a character match, character mismatch, gap
open and gap extend. The default payoffs are 1,-1,-1,-1.

=item B<-s>

Dump the scoring matrix

=item B<-t>

Run a self-test.
The output should be aligned.
...
=for HTML <a
href="http://www.psc.edu/biomed/TUTORIALS/SEQUENCE/DBSEARCH/tutorial.html">A
Tutorial on Searching Sequence Databases and Sequence Scoring Methods</a>
...
It may be copied and/or modified under the terms of the Artistic License.
