# Read data
library(foreign)
D <- read.spss("data.spss", to.data.frame=TRUE)
X <- D[,-(1:3)]
X <- X[,-46]

# Data exploration using globaltest (three times a pairwise comparison)
library(globaltest)
...
# load the lasso:
library(penalized)
# Make random train/test set.
# Set the seed (drawn randomly) for reproducability
set.seed(-273185307)
...
TR1 <- X[train & D$Donor %in% c("TB", "TST+"),] # extract TB and TST+ only
resp1 <- factor(D$Donor[train & D$Donor %in% c("TB", "TST+")])
res1 <- profL1(resp1, TR1, standardize=FALSE, minlambda1=.2) # plot cross-validated likelihood as a function of lambda
plot(res1$lambda, res1$cvl, type="l")
best1 <- optL1(resp1, TR1, standardize=FALSE) # optimize cross-validated likelihood as a function of lambda
coef(best1$full) # extract the coefficients
TST1 <- X[test & D$Donor %in% c("TB", "TST+"),] # get the test set
predictions1 <- predict(best1$full, TST1) # test set predictions
truth1 <- D$Donor[test & D$Donor %in% c("TB", "TST+")] # the truth for the test set
plot(truth1, predictions1) # ROC curve
...
res3 <- profL1(resp3, TR3, standardize=FALSE, minsteps=70) # optimum rond lambda=0 ??
plot(res3$lambda, res3$cvl, type="l")
...
# TB vs TST+
best1R <- optL2(resp1, TR1, standardize=FALSE)
coef(best1R$full)
...
AUC1R
# TB vs TSTbest2R
<- optL2(resp2, TR2, standardize=FALSE)
coef(best2R$full)
...
AUC2R
#TST+ vs TSTbest3R
<- optL2(resp3, TR3, standardize=FALSE)
coef(best3R$full)
