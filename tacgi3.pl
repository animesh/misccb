...
/* $Id: tacgi3.pl,v 1.3 2001/10/15 19:12:22 hjm Exp $  */
 
# usage: this is a cgi that is supposed to respond to input provided by 
# the tacg3xx.form.html page.

#these vars have to be set for each server setup.
##############################################################################
$server = "24.0.123.214";                   # What YOUR server is
$htmlpathtoform = "tacg3/tacg3.form.html";  # Where/what the form is
$pathtotacgdata = "/usr/local/lib/tacg3";   # Whre the data is - can probably just use the environment now
$webpathtodata = tacg3;                     # where your base httpd tacg3 dir is
$tmppath = "/tmp";                          # Where you want to store your temp stuff
$tacgapp = "/usr/local/bin/tacg3";          # The app name of the tacg3 exe
$ENV{TACGLIB} = "/usr/local/lib/tacg3/";    # the environment variable for TACGLIB
$QUEUE = "/usr/local/bin/queue";            # if using gnu queue
##############################################################################
# Version now taken care of by grabbing it out of the tacg executable itself
# $tacgversion = "3.10";                      # Duh.
...
#Open the tacgi3 error log file 
...
#   print "<i>$PASTEIN_SEQ</i><br>";
   ...
   } elsif ($SEQFILE = param('FUPLOAD_SEQ')) {
...
# But 1st!!  get the basic app working so I can start advertising it.

#   print "<i>PASTEIN_SEQ</i><br>";

print "<font size=5 color=\"red\">WWWtacg 3 Analysis</FONT><P>",
...
if (!defined $SEQFILE) {
   print "<BR>&#187; For pasted-in sequence, labelled <B> $PASTEDNAME </B>";
} else {
...
# How is $FONT dealt with in tacgi2?  Must be on output..
# Yup - just use the value of $FONT and repeat the $FONT # of '<SMALL>'s before output 
