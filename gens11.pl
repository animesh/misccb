...
#!/usr/bin/perl
if( @ARGV ne 4){die "\nUSAGE\t\"ProgName MultSeqFile windowlength ?baseperiodicity E-Icutoff\"\t\n\n\n";}
$file = shift @ARGV;
$ws = shift @ARGV;
$n= shift @ARGV;
$eicuto=shift @ARGV;
...
#open(FS1,">ecolmorethen4");
#open(FS2,">ecollessthen4");
...
$len=($N-$ws+1);
	...
	#print "$subseq\t$a\t$t\t$g\t$c\n";
		...
		if($ptnr >= 4.0)
			...
			print FS2"ELSE$c2 to $ll of length $ws is $ptnr\n";
			}
	}
	$flag=0;
