...
GetOptions("help","version","user=s","password=s",
	   "db_engine=s","db=s","host=s","max_mail_size=s") || usage();
...
sub date_parser
{
  ...
  $date_raw =~ /.*?(\d{1,2}?) (.+?) (\d{2,4}?) (\d\d:\d\d:\d\d?) ([\w-+]{3,5})/;
...
sub update_table
{
  ...
  {
      print "Aborting: Got error '$dbh->errstr' for query: '$query'\n";
  }
  else
  {
    $count_duplicates++;    
