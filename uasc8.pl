...
#push(@combos,1);
#for($com=0;$com<=$#combos;$com++){
$strin=pop(@uniq);
#print @combo;
#foreach $u1 (@combos){if($u1 eq $a2){next;}
#while(
@val=sort values %thresh;
@nval=reverse @val;
foreach $tr2 (@nval) {foreach $tr1 (keys %thresh) {
	if($tr2 eq $thresh{$tr1}){$hresh{$tr1}=$tr2;}
}}
foreach $uu (keys %hresh){print "$uu=>$hresh{$uu}\n";}
foreach $a1 (keys %thresh){foreach $a2 (keys %thresh)
{
#print "$a1.$a2\n";
push(@combos,$a2);
if(($a1 ne $a2) and (($thresh{$a1}>=$threshold) and ($thresh{$a2}>=$threshold))){print "$a1.$a2\n";
print FO"$a2\t";
print FD"$a2\t";
for($d3=1;$d3<=$tots3;$d3++){
for($d2=1;$d2<=$tots2;$d2++){
if($plan[$d3][0] eq $a2) {print FO"$plan[$d3][$d2]\t";}
}if($plan[$d3][0] eq $a2) {print FO"$thresh{$a2}\n";print "storing DISTANCE MATRIX\t".$d3."\t";}
}
print "\n";
for($d3=1;$d3<=$tots3;$d3++){
for($d2=1;$d2<=$tots2;$d2++){
if($plan[$d3][0] eq $a2) {print FD"$dist[$d3][$d2]\t";}
}if($plan[$d3][0] eq $a2) {print FD"$thresh{$a2}\n";print "storing POSITION MATRIX\t".$d3."\t";}
}
}
else{next;}
}
...
#print "$uu : $thresh{$uu}\n";
#}
#foreach $uuu (keys %thresh){print "$uuu : $thresh{$uuu}\n";}
#print "$threshold\n";
#}