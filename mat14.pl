...
#!/usr/bin/perl
print "\nEnter the nucleotide Sequence 1\t";
$sequence1=lc(<>);
chomp $sequence1;
@seq1=split(//,$sequence1);
print "\nEnter the nucleotide Sequence 2\t";
$sequence2=lc(<>);
chomp $sequence2;
@seq2=split(//,$sequence2);
print "\nEnter the Match Value(default=2)\t";
$match=<>;
print "\nEnter the Mismatch Value(default=0)\t";
$mismatch=<>;
print "\nEnter the Partial Match Value\n(Purine to Purine AND
Pyrimidine to Pyrimidine;Default=1\t";
$pmatch=<>;
...
$compf[$cc][0]=@seq2[$l];}
$comp[0][0]=0;$comp[1][1]=0;$compf[0][0]=0;$compf[1][1]=0;
for ($gg=2; $gg<=$len1;$gg++)
 { $co1 = $gap+$comp[1][($gg-1)];
 $comp[1][$gg]=$co1;
 $compf[1][$gg]=9; }
...
if($co2>$max){$comp[$b][$a]=$co2;$flag=4;$compf[$b][$a]=$flag;$max=$co2;if($co2
eq $co3)
  {$flag=13;$compf[$b][$a]=$flag;}}

if($co3>$max){$comp[$b][$a]=$co3;$flag=9;$compf[$b][$a]=$flag;$max=$co3;if($co1
eq $co3)
