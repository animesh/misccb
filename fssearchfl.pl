#!/usr/bin/perl
# fssearchfl.pl - Feasibility search follow-up letter
...
print $query->header ();
	...
	###########################################
    	...
    	#print $query->start_html(-bgcolor => "#FFF8DF", -title=>'Feasibility Study - Search', -onLoad=>"document.frm.fid.focus();");
...
#	$where = "WHERE " . join(" AND ", @condition) if @condition;
	...
	push (@searchcriteria, Tr (td({-bgcolor => "#0000FF", -colspan => "12"}, 
				...
				strong("Predefined Feasibility Study Search (Follow-up Letters) Criteria")))));
...
#       push(@searchcriteria,Tr (td({-colspan => "12"},span({-style => 'font-family: arial;font-size: 12'}, "&nbsp"  ,$sth->rows(), " Feasiblity search record(s) found"))));
	...
	if ($ng ne "")
	{
		...
		push (@row, Tr (td({-bgcolor => "#0000FF", -colspan => "9"}, "&nbsp",
				span({-style => 'font-family: arial;color :"#FFFFFF";font-size: 12'},
	...
	else
	{
		...
		push(@str,Tr  (
			td({-bgcolor => "#0000FF",-colspan=>"12"},
				span({-style => 'font-family: arial;color : "#FFFFFF";font-size: 12;'}, b("Predefined Feasibility Study Search Results (Follow-up Letters)"))),
				
			Tr(td({-colspan=>"12"},span({-style => 'font-family: arial;font-size: 12;'}, "The search results are refined by listing only the studies of which not all doctors (who have returned feasibility study forms) have received 'Follow-up Letters (Successful or Unsuccessful)'")))
		));
##marking
		my $fidcount=0;
		while (my $myref = $sth->fetchrow_hashref ())
...
#			my $checkquery="select fid from fsform where fid='$myref->{fid}' and (lsent like '%2%' or lsent like '%3%')";
			my $checkquery="select fid from fsform where fid='$myref->{fid}' and dfrecd !='0000-00-00' and lsent not like ('%2%') and lsent not like ('%3%') ";
#			print p($checkquery);
			...
			while ( $fsref = $checkprepare->fetchrow_hashref ())
			{
				...
				push(@str,Tr (
					...
					td ($align,span({-style => 'font-family: arial;font-size: 12'}, get_disp_date($myref->{dtrecd}))),
					#td ($align, "&nbsp", span({-style => 'font-family: arial;font-size: 12'}, ($myref->{dtrecd}))),
					td ({-align=>"right",-bgcolor => "#87CEEB"},$align,span({-style => 'font-family: arial;font-size: 12'}, "Successful Bid :")),
...
#					td ({-colspan=>11,-width=>"88%"},$align, "&nbsp", span({-style => 'font-family: arial;font-size: 12'},@newarray))
		...
		print "<br/>";
            	push(@searchcriteria,Tr (td({-colspan => "12"},span({-style => 'font-family: arial;font-size: 12'}, "&nbsp"  ,$fidcount, " Feasibility record(s) found"))));
		...
		if ($fidcount > 0)
			...
			print "<a class='txt' href=\"fssearchfl.pl\" style = 'font-family: arial;font-size: 12'><u>Predefined Feasibility Study Search (Follow-up Letters)</u>";
			print "</td></tr></table>";
...
sub show_dr_form
{
		...
		table   ({-width => "100%", -cellspacing => "2", -cellpadding => "1", -border => "1", -rules=>"all"},
			...
			Tr (td({-bgcolor => "#0000FF", -colspan => "7"},
				...
				strong("Predefined Feasibility Study Search (Follow-up letters)")))
			),

			Tr (
			
				td({-colspan => "7"},span({-style => 'font-family: arial;font-size: 14'},"Indicate the criteria below to search for feasibility studies. The search results is further refined by listing only the studies of which not all doctors (who have returned feasibility study forms) have received 'Follow-up Letters (Successful or Unsuccessful)'"))
			...
			Tr (
				td ({-width=>"20%",-align=>"right",-bgcolor=>"#87CEEB",-valign => "top"}, "&nbsp",
				 ...
				 td ({-width=>"80%"},textfield(-name => "fid", -size => "40", -maxlength => "35", -value => "$fid")),
			),
			Tr (
...
#
#			Tr (
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#					span({-style => 'font-family: arial;font-size: 12'},"Reference ID:")),
#				 td ({-colspan=>2},textfield(-name => "fid", -size => "40", -maxlength => "35", -value => "$fid")),
#			),
#			Tr (
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#					span({-style => 'font-family: arial;font-size: 12'},"Sponsor:")),
#				td({-colspan=>2},textfield (-name => "sname", -size => "60", -maxlength => "50", -value => "$sname"))
#			),
#			Tr (
#
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#					span({-style => 'font-family: arial;font-size: 12'},"Therapeutic Area:")),
#				td({-colspan=>2},popup_menu(-name => "tid", -values => \@tname_val_ref, -default => "$tid", -onchange =>"document.frm.submit()"))
#
#			),
#			Tr (
#
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#					span({-style => 'font-family: arial;font-size: 12'},"Indication:")),
#				td({-colspan=>2},popup_menu(-name => "inid", -values => \@iname_val_ref, -default => ""))
#
#			),
#			Tr (
#
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#					span({-style => 'font-family: arial;font-size: 12'},"Product:")),
#				td({-colspan=>2},textfield(-name => "product", -size => "60", -maxlength => "50", -value => "$product"))
#
#			),
#			Tr (
#
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#					span({-style => 'font-family: arial;font-size: 12'},"Quintiles Contact / Q Office:")),
#				td({-colspan=>2},textfield(-name => "ocontact", -size => "60", -maxlength => "50", -value => "$ocontact"))
#				#td(textfield(-name => "ocontact", -size => "60", -maxlength => "50", -value => "$ocontact"))
#
#			),
#			
...
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5},"&nbsp",
#				        span({-style => 'font-family: arial;font-size: 12'},"Local BD Contact First Name :")),
#				td({-colspan=>2},textfield (-name => "contactfname",	-value => "$contactfname", -size => 30))
#			),
#			
#			Tr (
#			
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#				        span({-style => 'font-family: arial;font-size: 12'},"Local BD Contact Last Name :")),
#				td({-colspan=>2},textfield (-name => "contactlname",	-value => "$contactlname", -size => 30))
#			),		
#			
#			
#			
...
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#					span({-style => 'font-family: arial;font-size: 12'},"Due Date:")),
#				td({-colspan=>2},textfield(-name => "duedate", -size => "60", -maxlength => "50", -value => "$duedate"))
#
#			),
#			Tr (
#
#				td ({-align=>"right",-bgcolor=>"#87CEEB",-valign => "top",-colspan=>5}, "&nbsp",
#					span({-style => 'font-family: arial;font-size: 12'},"Blind Study:")),
#				td({-colspan=>2},popup_menu(-name => "bstudy", -values => ['Any','Yes','No'], -default => "$bstudy" ))
#
#			),
#
#		),
