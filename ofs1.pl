...
sub LEARNING{$iterno=0;$invsqerr=$class*$samples;
	while(($iterno < $iter) and ( $invsqerr >= 0.05 )){
		$squareerror =0;
...
#				}
#				$gamma-=($eta*$data[$datarow][$i])*(1/(1+exp(-($gamma))))*(1-(1/(1+exp(-($gamma)))));
#				$errorgamma=0;
#			}
		 }
	$iterno++;
...
$temp1=0;
   for($ic1=0;$ic1<$features;$ic1++){
      $temp1+=$weight12[$ic1][$hiddenUnitID]*$data[$datarow][$ic1]*(1/(1+exp(-($gamma))));
	  #print "@ofs[$ic1]";
	  #/*atten[n];*/
   }
